...
The idea is as follows: the `NeuralNetwork` class is like a little math whiz learning to solve logic gate problems.
...
And voila! The wizard has mastered the logic gates! Now he can help you solve logic problems with his math magic!

The `NeuralNetwork` class uses the sigmoid activation function, defined as: $$\sigma(x) = \frac{1}{1 + e^{-x}}$$ to transform the values of neurons in the network.
...
The weights are then updated using the formula: $$w_{+} = w + \eta \cdot \text{input} \cdot \text{error} \cdot \sigma'(\text{output})$$ where $\eta$ is the learning rate and $\sigma'(x)$ is the derivative of the sigmoid function, defined as: $$\sigma'(x) = x \cdot (1 - x)$$
